...
async function main() {
    ...
    await db.queryPromise(
        `CREATE TABLE ?? (
            ...
            boe_zip CHAR(5),
            new_voter_id INT UNSIGNED UNIQUE
        )`,
        [tempTable]
    );
     */
        ...
        FROM petition_lines l
            ...
            INNER JOIN ?? v2
                ON v.registered = v2.registered AND
                    ...
                    v.suffix = v2.suffix
            LEFT JOIN mailing_i81 m ON m.new_voter_id = v2.voter_id
            LEFT JOIN zottoli z ON z.new_voter_id = v2.voter_id
        WHERE finding <> '' AND l.project_id = ? AND v2.ward = 4
            AND m.new_voter_id IS NULL AND z.new_voter_id IS NULL`,
        [oldVotersTable, newVotersTable, projectId]
    );
