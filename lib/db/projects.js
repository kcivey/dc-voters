module.exports = function (db) {
    ...
    db.updateProject = function (id, projectData) {
        delete projectData.id;
        delete projectData.code;
        if (projectData.findingCodes) {
            projectData.findingCodes = JSON.stringify(projectData.findingCodes, null, 2);
        }
        if (projectData.circulatorStatuses) {
            projectData.circulatorStatuses = JSON.stringify(projectData.circulatorStatuses, null, 2);
        }
        const sql = 'UPDATE projects SET ? WHERE id = ?';
        const values = [projectData, id];
        return queryPromise(sql, values)
            .then(() => db.getProjectById(id));
    };
    ...
    db.getProjectById = function (id) {
        return queryPromise('SELECT * FROM projects WHERE id = ?', [id])
            .then(rows => (rows[0] ? adjustProject(rows[0]) : null));
    };
...
function adjustProject(rawProject) {
    ...
    if (project.batches) {
        project.batches = project.batches.split(/\s*,\s*/)
            .map(n => +n);
    }
