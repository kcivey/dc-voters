    ...
    async function syncVoterIds() {
        ...
        await queryPromise(
            ...
            SELECT MAX(voter_id) AS voter_id,
                registered,
                LEFT(firstname, 20) AS firstname,
                LEFT(lastname, 20) AS lastname,
                LEFT(middle, 1) AS middle,
                suffix,
                res_house,
                res_frac,
                LEFT(res_street, 25) AS res_street,
                res_apt
            FROM ${oldVotersTable}
        ...
        await queryPromise(
            ...
            SET voter_id = (
                ...
                WHERE v1.registered = v2.registered AND
                    LEFT(v1.firstname, 20) = v2.firstname AND
                    LEFT(v1.lastname, 20) = v2.lastname AND
                    LEFT(v1.middle, 1) = v2.middle AND
                    v1.suffix = v2.suffix AND
                    v1.res_house = v2.res_house AND
                    v1.res_frac = v2.res_frac AND
                    LEFT(v1.res_street, 25) = v2.res_street AND
