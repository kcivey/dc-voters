...
db.createDatabaseAndUser = async function (dbName = 'dc_voters', username = dbName, password = null) {
    if (!password) {
    ...
    await queryPromise('CREATE USER ?@localhost IDENTIFIED BY PASSWORD ?', [username, password]);
    await queryPromise('GRANT DELETE, INSERT, SELECT, UPDATE ON ??.* TO ?@localhost');
