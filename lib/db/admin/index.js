...
db.loadVoterFile = async function (filename) {
    await queryPromise('DROP TABLE IF EXISTS voters_temp');
    await queryPromise('CREATE TABLE voters_temp LIKE voters_base');
    await queryPromise(
        `LOAD DATA LOCAL INFILE ? INTO TABLE voters_temp
    ...
    const maxRegistrationDate = await queryPromise(
        `SELECT MAX(v.registered) AS max_registered FROM
            (SELECT registered, COUNT(*) FROM voters_temp GROUP BY registered HAVING COUNT(*) > 10) v`
    ).then(rows => rows[0].max_registered);
    const votersTable = 'voters_' + maxRegistrationDate.replace(/-/g, '');
    await queryPromise('RENAME TABLE voters_temp TO ??', [votersTable]);
    return votersTable;
};

db.userDatabase = function (database) {
