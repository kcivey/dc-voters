...
module.exports = function (db) {
    ...
    db.getLineCountsForCirculator = function ({project, circulatorId, startDate = null, endDate = null}) {
        let sql =
    ...
    db.getLineCountsForCirculatorByWard =
        function ({project, circulatorId, startDate = null, endDate = null, unpaid = false}) {
            let sql =
                `SELECT v.ward, COUNT(*) AS valid
                FROM petition_lines l
                    INNER JOIN pages p ON l.project_id = p.project_id AND l.page = p.number
                    INNER JOIN ?? v ON l.voter_id = v.voter_id
                WHERE l.project_id = ? AND p.circulator_id = ? AND l.finding = 'OK'`;
            const values = [project.votersTable, project.id, circulatorId];
            if (startDate) {
                sql += ' AND p.date_checked >= ?';
                values.push(startDate);
            }
            ...
            sql += ' GROUP BY v.ward ORDER BY v.ward';
            return queryPromise(sql, values);
        };
