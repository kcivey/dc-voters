...
module.exports = function (db) {
    ...
    db.getCirculatorsWhoAreOwed = function (project, date) {
        return queryPromise(
            'SELECT * FROM circulators WHERE id IN ' +
                '(SELECT DISTINCT circulator_id FROM pages ' +
                'WHERE project_id = ? AND processed_lines = ? AND date_received <= ? AND invoice_id IS NULL)',
            [project.id, project.linesPerPage, date]
        );
    };
