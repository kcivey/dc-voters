...
module.exports = function (db) {
    ...
    db.createOrUpdateUser = async function (projectId, userData, id = null) {
        ...
        let sql;
        if (!id) {
            const user = await db.getUser({email: userData.email});
            if (user) {
    ...
    db.getUsersForProject = function (project) {
        const subquery =
            `SELECT checker, AVG(diff) AS avg_time
            FROM (${db.avgTimeSql}) t
            WHERE diff <= ${db.avgTimeMax} AND finding <> 'B'
            GROUP BY checker`;
        const assignedLinesCount = project.type === 'response' ? 'SUM(IF(l.challenged, 1, 0))' : 'COUNT(l.finding)';
        const sql =
            `SELECT u.*,
                ${assignedLinesCount} AS assigned_lines,
                SUM(IF(l.finding NOT IN ('', 'S'), 1, 0)) AS processed_lines,
            ...
            GROUP BY u.id
            ORDER BY u.username`;
        return queryPromise(sql, [project.id, project.id])
            .then(function (rows) {
                rows.forEach(function (row) {
                    row.pages = row.pages ? NumberList.stringify(row.pages.split(',').filter(n => n !== '')) : '';
                });
                return rows;
            });
    };
