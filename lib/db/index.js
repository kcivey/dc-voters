...
db.getTotals = function (project, circulatorId = null) {
    ...
    async function getWardRegistered() {
        const wardRegistered = {TOTAL: 0};
        if (Array.isArray(project.voterCounts)) {
            let ward = 0;
            for (const count of project.voterCounts) {
                ward++;
                wardRegistered[ward] = count;
                wardRegistered.TOTAL += count;
            }
        }
        else {
