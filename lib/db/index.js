...
db.establishConnection = function (config = {}) {
...
db.getTotals = function (projectId, circulatorId = null) {
    ...
    function getAvgTime() {
        return queryPromise(`SELECT AVG(diff) AS avg_time FROM (${db.avgTimeSql}) t WHERE diff < ${db.avgTimeMax}`)
    ...
    return Promise.all([getCountsByFinding(), getWardRegistered(), getWardSigners(), getAvgTime()])
        .then(function (results) {
            const [totals, registedByWard, signersByWard, avgTime] = results;
            const wardBreakdown = {};
            for (const [key, count] of Object.entries(registedByWard)) {
            ...
            totals['Avg sec per line'] = avgTime.toFixed(1);
