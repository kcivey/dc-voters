...
module.exports = function (db) {
    ...
    db.getPages = function (projectId) {
        const sql = 'SELECT p.*, c.name AS circulator_name, i.date_paid, ' +
    ...
    db.updateProcessedLines = function (projectId) {
        const sql = 'UPDATE pages p ' +
            ...
            'SET valid_lines = (SELECT COUNT(*) FROM petition_lines l WHERE project_id = ? AND l.page = p.number ' +
                "AND finding = 'V' " +
            'WHERE project_id = ? AND processed_lines < 20';
        return queryPromise(sql, [projectId, projectId, projectId]);
    };

    db.getUnpaidPagesForCirculator = function (projectId, circulatorId, date) {
        const sql = 'SELECT * FROM pages WHERE project_id = ? AND circulator_id = ? AND date_received <= ? ' +
            'AND processed_lines = 20 AND invoice_number IS NULL';
        return queryPromise(sql, [projectId, circulatorId, date]);
    };
