...
module.exports = function (db) {
    ...
    db.getPage = function (projectId, pageNumber) {
        return db.getPages(projectId, pageNumber)
            .then(rows => rows[0]);
    };

    db.getPages = function (projectId, pageNumber) {
        ...
        sql += ' GROUP BY p.number ORDER BY p.number';
        return queryPromise(sql, values);
    };

    db.updateProcessedLines = function (project) {
        const projectId = project.id;
        const sql = 'UPDATE pages p ' +
            'SET processed_lines = (SELECT COUNT(*) FROM petition_lines l WHERE project_id = ? AND l.page = p.number ' +
                "AND finding NOT IN ('S', '')), " +
            'valid_lines = (SELECT COUNT(*) FROM petition_lines l WHERE project_id = ? AND l.page = p.number ' +
                "AND finding = 'V') " +
            'WHERE project_id = ? AND processed_lines < ?';
        return queryPromise(sql, [projectId, projectId, projectId, project.linesPerPage]);
    };

    db.getUnpaidPagesForCirculator = function (project, circulatorId, date) {
        const sql = 'SELECT * FROM pages WHERE project_id = ? AND circulator_id = ? AND date_received <= ? ' +
            'AND processed_lines = ? AND invoice_number IS NULL';
        return queryPromise(sql, [project.id, circulatorId, date, project.linesPerPage]);
    };
